window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function createFx() {
			...
			let ripples = true;    // 波纹开关：只控制鼠标涟漪，背景网格属于皮肤底色，始终保留
			let attached = false;  // 鼠标事件是否已挂上
			...
			function spawn(x, y, power, scale) {
				if (reduce || !ripples || !host || !wctx) return;
			...
			function detach() {
				...
				lastX = -1e5; lastY = -1e5;   // 复位，重新开启时不会立刻补一朵
				waves.length = 0;
				if (wctx && waveCv) wctx.clearRect(0, 0, W, H);
			...
			return {
				/** gridOn：是否挂载特效层（背景网格）；rippleOn：是否产生鼠标波纹 */
				set: function (gridOn, rippleOn, st) {
					strength = st || 1;
					ripples = !!rippleOn;
					if (gridOn) { if (!host) mount(); else { build(); if (ripples) attach(); else detach(); } }
					else if (host) unmount();
				},
				destroy: unmount,
		...
		function applyState() {
			...
			body.dataset.dshFx = state.on ? "on" : "off";   // 特效层（背景网格）跟随皮肤总开关
			body.dataset.dshRipple = state.on && state.fx ? "on" : "off";
			if (state.on && state.fxTop) body.dataset.dshFxTop = "1"; else delete body.dataset.dshFxTop;
			fx.set(state.on, state.on && state.fx, state.strength);
		}
		//#endregion
		...
		function SkinSection() {
			...
			return jsx("div", {
				...
				children: [
					...
					jsx(Row, { title: "波纹特效", hint: "关闭后仅去掉鼠标涟漪，背景的六边形网格保留", children: onOff("fx", state.fx) }),
					jsx(Row, {
						title: "波纹强度",
						hint: "影响波纹的不透明度",
						children: [
